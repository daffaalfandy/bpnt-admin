...
const getters = {
  ...
  sumOfPrice: (state) => state.sumOfPrice,
};

const actions = {
  async changeDatepick({ commit }, payload) {
  ...
  async getTransactionBasedOnDate({ commit }, payload) {
    const response = await axios.get(
      `${url}/all?d=${payload.date}&m=${payload.month}&y=${payload.year}`
    ); // eslint-disable-line no-unused-vars

    commit("addTransaction", response.data);
  },

  async getTransactionBasedOnMonth({ commit }, payload) {
    const response = await axios.get(
      `${url}/all?m=${payload.datepick.month}&y=${payload.datepick.year}`
    ); // eslint-disable-line no-unused-vars
...
const mutations = {
  ...
  updateSumOfPrice: (state, payload) => (state.sumOfPrice += payload),
  emptySumOfPrice: (state) => (state.sumOfPrice = 0),
};
